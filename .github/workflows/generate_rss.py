...
CHANNEL = "freekvn"  # сюда можно вписать любой публичный канал
...
def fetch_posts():
    try:
        ...
        return []
    # Достаём блоки с постами
    raw_posts = re.findall(r'<div class="tgme_widget_message_text js-message_text".*?</div>', resp.text, re.DOTALL)
    posts = []
...
def build_rss(posts):
    items = []
    for i, post in enumerate(posts[:5]):  # берём 5 последних
